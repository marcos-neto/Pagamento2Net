...
{
    internal class OcorrênciasFebraban : OcorrênciaParser
    {
        ...
        public override KeyValuePair<string, string> Parse(string code)
        {
            if (Ocorrências.TryGetValue(code, out string description))
            {
                return new KeyValuePair<string, string>(code, description);
            }
            else
            {
                throw new KeyNotFoundException($"{code} - Código de ocorrência não encontrado.");
            }
        }
    }
}